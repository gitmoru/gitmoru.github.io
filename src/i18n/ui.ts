...
export const ko = {
  ...
  home: {
    ...
    when: [
      ...
      '사고가 없을 때 정기 점검. 배포 키, 웹훅, self-hosted 러너는 푸시 기록에 안 남아서 따로 봐야 해요',
...
https://gitmoru.dev`,
    ...
    stuck: [
      ...
      ['문단속이 대부분 안 보여요', '배포 키, 웹훅, 러너, 비밀은 저장소 관리자여야 조회됩니다. 권한이 없으면 "못 봤다" 고 정직하게 나와요. 그걸 "없다" 로 읽으면 안 됩니다. 다만 저장소가 공개로 바뀐 것, 포크된 것, 사람이 추가된 것은 권한 없이도 보입니다.'],
...
확인하지 못한 푸시 1건 - 없었다는 뜻이 아닙니다`,
    ...
    tools: [
      ...
      ['check_access', '배포 키, 웹훅, 대기 초대, self-hosted 러너, 비밀이 바뀐 시각'],
...
export const en: Dict = {
  ...
  home: {
    ...
    when: [
      ...
      'Nothing happened, and you are doing a routine check. Deploy keys, webhooks and self-hosted runners leave no push trail',
...
https://gitmoru.dev`,
    ...
    stuck: [
      ...
      ['The access check comes back mostly empty', 'Deploy keys, webhooks, runners and secrets need admin on the repo. Without it the tool says so plainly. Do not read that as "there are none". Repositories going public, being forked, and people being added show up with no permissions at all.'],
...
1 push could not be verified, which does not mean it did not happen`,
    ...
    tools: [
      ...
      ['check_access', 'Deploy keys, webhooks, pending invites, self-hosted runners, when secrets changed'],
...
export const ja: Dict = {
  ...
  home: {
    ...
    when: [
      ...
      '事故がないときの定期点検。デプロイキー、Webhook、セルフホストランナーはプッシュ履歴に残りません',
...
https://gitmoru.dev`,
    ...
    stuck: [
      ...
      ['入り口の点検がほとんど空で返る', 'デプロイキー、Webhook、ランナー、シークレットにはリポジトリの管理者権限が要ります。権限がなければその旨をそのまま出します。それを「無い」と読まないでください。なお、公開への変更、フォーク、メンバー追加は権限なしでも見えます。'],
...
確認できなかったプッシュ 1 件。なかったという意味ではありません`,
    ...
    tools: [
      ...
      ['check_access', 'デプロイキー、Webhook、保留中の招待、セルフホストランナー、シークレットの更新時刻'],
